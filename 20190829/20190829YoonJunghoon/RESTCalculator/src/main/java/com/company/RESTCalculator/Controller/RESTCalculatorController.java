...
public class RESTCalculatorController {
    ...
    public int add(@Valid @RequestBody RESTCalculator calc)  {
        return calc.add(calc.getOperand1(), calc.getOperand2());
    }

    @PostMapping(value = "/subtract")
    @ResponseStatus(value = HttpStatus.CREATED)
    public int subtract(@Valid @RequestBody RESTCalculator calc) {
        return calc.subtract(calc.getOperand1(), calc.getOperand2());
    }

    @PostMapping(value = "/multiply")
    @ResponseStatus(value = HttpStatus.CREATED)
    public int multiply(@Valid @RequestBody RESTCalculator calc) {
        return calc.mult(calc.getOperand1(), calc.getOperand2());
    }

    @PostMapping(value = "/divide")
    @ResponseStatus(value = HttpStatus.CREATED)
    public int divide(@Valid @RequestBody RESTCalculator calc) {
